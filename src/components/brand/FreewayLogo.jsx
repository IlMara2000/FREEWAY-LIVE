...
export default function FreewayLogo({ className = '', iconClassName = '', showWordmark = false }) {
  ...
  const bgId = `freeway-logo-bg-${id}`;
  const laneId = `freeway-logo-lane-${id}`;
  ...
  return (
    <span className={`inline-flex items-center gap-3 ${className}`}>
      <svg
        viewBox="0 0 64 64"
        aria-hidden="true"
        className={`h-10 w-10 shrink-0 drop-shadow-[0_0_18px_rgba(45,212,191,0.42)] ${iconClassName}`}
      >
        <defs>
          <linearGradient id={bgId} x1="9" y1="5" x2="55" y2="60" gradientUnits="userSpaceOnUse">
            <stop offset="0" stopColor="#0f2130" />
            <stop offset="0.48" stopColor="#02050c" />
            <stop offset="1" stopColor="#04141d" />
          </linearGradient>
          <linearGradient id={laneId} x1="16" y1="52" x2="50" y2="10" gradientUnits="userSpaceOnUse">
            <stop offset="0" stopColor="#f59e0b" />
            <stop offset="0.34" stopColor="#34d399" />
            <stop offset="1" stopColor="#67e8f9" />
          </linearGradient>
        </defs>
        <rect x="5" y="5" width="54" height="54" rx="17" fill={`url(#${bgId})`} />
