...
export default function Home() {
  ...
  useEffect(() => {
    ...
    if (savedName) { setUsername(savedName); setIsLoggedIn(true); }
  }, []);

  const handleLogin = () => {
    if (inputValue.trim()) {
      localStorage.setItem('freeway_user', inputValue);
      setUsername(inputValue); setIsLoggedIn(true);
    }
  };
  ...
  if (!isLoggedIn) {
    return (
      <div className="min-h-screen flex items-center justify-center p-4">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="glass-panel p-10 rounded-[2rem] w-full max-w-md text-center">
  ...
  return (
    <div className="min-h-screen pb-32 p-4 md:p-8">
      <header className="flex justify-between items-center mb-8 max-w-[1400px] mx-auto px-2">
        <div>
          <h1 className="text-2xl md:text-3xl font-black text-white tracking-widest">FREEWAY<span className="text-emerald-500 text-4xl leading-none">.</span>LIFE</h1>
        ...
        <button onClick={handleLogout} className="text-[10px] font-mono text-zinc-500 hover:text-red-400 transition-colors tracking-widest uppercase">Disconnetti</button>
      </header>

      {/* Grid a 12 colonne per precisione assoluta (4 a sinistra, 8 a destra) */}
      <main className="max-w-[1400px] mx-auto grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8 px-2">
        <div className="flex flex-col gap-6 lg:gap-8 lg:col-span-4">
          <Tomato />
          <CalendarWidget />
        </div>
        <div className="flex flex-col gap-6 lg:gap-8 lg:col-span-8 h-full">
          <Planner />
        </div>
      </main>
      <MobileNav />
