...
export default function CalendarView({ onStartTomato }) {
  ...
  const getTasksForDay = (day) => {
    ...
    return tasks
      .filter((task) => task.due_date === dateStr || (task.status === 'today' && !task.due_date && day === today.getDate() && month === today.getMonth() && year === today.getFullYear()))
  ...
  const handleDayClick = (day) => {
    ...
    if (getTasksForDay(day).length === 0) {
      setCreateDate(getDateString(day));
    }
  };
  ...
  return (
    ...
    >
      <div className="flex items-end justify-between gap-4 pt-2">
        <div>
          <p className="font-mono text-[10px] text-emerald-400/60 tracking-widest uppercase mb-1">Calendario turni</p>
            ...
            {cells.map((day, i) => {
              ...
              return (
                <motion.button
                  ...
                  className={`group relative min-h-[86px] md:min-h-[118px] rounded-2xl p-2.5 flex flex-col items-stretch transition-all text-left overflow-hidden ${
                    active
                ...
                >
                  ...
                  <div className="mt-2 space-y-1 overflow-hidden">
                    {dayTasks.length > 0 ? dayTasks.slice(0, 3).map((task) => {
                      ...
                      <span className="mt-auto text-[10px] text-white/20 opacity-0 group-hover:opacity-100 transition-opacity">+ turno</span>
                    )}
                    {dayTasks.length > 3 && (
                  ...
                  <span
                    ...
                    className="absolute right-1.5 top-1.5 w-6 h-6 rounded-lg bg-black/35 border border-white/10 text-white/45 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity"
                  >
